...
].map(name => `compactors/${slugify(name)}.html`);
...
].map(name => `racks/${slugify(name)}.html`);
...
].map(name => `lockers/${slugify(name)}.html`);
...
].map(name => `cabinets/${slugify(name)}.html`);
...
const projectPages = casesData.map(c => `projects/${slugify(c.title)}.html`);
...
].map(name => `blog/${slugify(name)}.html`);
...
const sitemapEntries = allRelativeUrls.map(rel => {
  ...
  else if (rel.startsWith('projects/') || rel.startsWith('blog/')) { priority = '0.7'; changefreq = 'monthly'; }
