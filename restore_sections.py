...
# Map should go before Featured Projects.
# FAQ should go after Featured Projects, before the footer.
# Footer should be replaced.

# Re-run the layout builder with all the pieces from current + the restored pieces from backup.
...
# Let's find where the scripts start or where the old footer starts to replace it as well.
# There is a problem: the current_html footer might be mismatched. Let's find the old <!-- FOOTER --> or the </footer> tag.
end_m = re.search(r'<!-- FOOTER -->', current_html)
if not end_m:
    end_m = re.search(r'<footer[^>]*>', current_html)

if start_m and end_m:
    # also we need to locate the end of the footer in current_html to completely replace it
    footer_end_m = re.search(r'</footer>', current_html[end_m.start():])
    
    # Slice the top of the HTML
    top_html = current_html[:start_m.start()]
    
    # Slice the bottom of the HTML (after the footer)
    if footer_end_m:
        bottom_html = current_html[end_m.start() + footer_end_m.end():]
    else:
        bottom_html = current_html[end_m.start():] # fallback
